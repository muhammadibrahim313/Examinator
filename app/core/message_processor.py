...
class MessageProcessor:
    """
    Main message processing engine that coordinates different handlers
    """
    ...
    def _setup_handlers(self):
        """Setup message handlers in priority order"""
        self.handlers = [
            GlobalCommandHandler(self.state_manager, self.exam_registry),
            ExamSelectionHandler(self.state_manager, self.exam_registry),
            ExamTypeHandler(self.state_manager, self.exam_registry),
            FallbackHandler()  # Always last
        ]
        logger.info(f"Initialized {len(self.handlers)} message handlers")
    ...
    def process_message(self, user_phone: str, message: str) -> str:
        ...
        try:
            ...
            # Process the message
            result = handler.handle(user_phone, message, user_state)
    ...
    def _find_handler(self, message: str, user_state: Dict[str, Any]) -> Optional[MessageHandler]:
        """
